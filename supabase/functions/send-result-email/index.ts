...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Generate a nicely formatted HTML email using Gemini Flash
    const prompt = `Generate a clean HTML email body (no <html>, <head>, <body> tags - just the inner content). 
...
- Image: ${image_url}`;
    ...
    // Try Gemini
    try {
      const aiResp = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${googleApiKey}`, {
        method: "POST",
      ...
      console.warn("Gemini email gen failed:", e.message);
    }
        ...
        try {
          const groqResp = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({
              ...
              messages: [
                { role: "system", content: "You are a professional agricultural report assistant." },
          ...
          console.error("Groq email fallback failed:", e.message);
        }
      }
    }
